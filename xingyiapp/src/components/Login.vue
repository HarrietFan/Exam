  ...
  <div class="shopping-classify">
    <div class="login-wrap">
      <el-form class="login-container">
        ...
        <el-form-item >
          <el-input type="text" v-model="user.username"  placeholder="用户账号" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item >
          <el-input type="password" v-model="user.password" placeholder="用户密码" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item>
  ...
  export default {
    ...
    methods: { //定义方法
      login() {
        ...
        // 发送请求，访问服务器接口
        this.$axios({
          url : 'user/login', //  请求的接口，因为配置 axios, 所以默认会加上 http://localhost:8080/
          method : 'post',
          data : JSON.stringify(this.user), // post请求，使用 data 属性给服务器传参，JOSN.stringify作用时把 js 对象转为 JSON字符串
        }).then(res => { // 请求成功的问题函数，res 就是服务器返回给前端的数据内容
          _this.$message(res.data.msg); // 弹出登录信息
          // 延迟函数：等待多久时间执行代码
          setTimeout(()=>{
            // 判断是否登录成功
            if(res.data.status === 200){
              // 登录成功
              // 1、把当前登录的用户名和头像路径存入本地存储中 sessionStorage
              sessionStorage.setItem('loginUsername',_this.user.username);
              sessionStorage.setItem('loginUserImage',res.data.data);
              // 2、页面跳转，跳转到 Index.vue
